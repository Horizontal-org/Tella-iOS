    ...
    var body: some View {
        VStack(spacing: 0){
            ...
            VStack(spacing: 0) {
                ...
                HStack(spacing: 0){
                    NavigationLink(destination: FileListView(files: VaultFile.stubFiles(), fileType: nil)) {
                        FileGroupView(groupName: "My Files", iconName: "files.my_files")
                            .padding(EdgeInsets(top: 0, leading: 0, bottom: 16, trailing: 8))
                    }
                    NavigationLink(destination: FileListView(files: viewModel.fileManager.root.files, fileType: .image)) {
                ...
                HStack(spacing: 0){
                    ...
                    NavigationLink(destination: FileListView(files: viewModel.fileManager.root.files, fileType: .document)) {
...
struct FileGroupsView_Previews: PreviewProvider {
    static var previews: some View {
        FileGroupsView(viewModel: HomeViewModel())
    }
}
