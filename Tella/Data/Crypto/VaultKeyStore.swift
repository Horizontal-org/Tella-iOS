...
final class VaultKeyStore: VaultKeyStoring {
    ...
    init(
        ...
        let pairStore = vaultKeyPairStore ?? VaultKeyPairStore(keychainStore: keychainStore)
    ...
    func setup(type: PasswordTypeEnum, password: String) throws -> SecKey {
        ...
        do {
            ...
            let metaPrivateKey = try keychainStore.create(
                passwordType: type,
                keyID: keyID,
                password: password
            )
    ...
    private func recoverVaultPrivateKey(password: String?) -> SecKey? {
        ...
        guard let metaPrivateKey = keychainStore.recoverMetaPrivateKey(
            keyID: keyID,
            password: password
        ) else {
    ...
    private func recoverStoredPrivateKeyData() -> Data? {
        ...
        if let data = keychainStore.recoverEncryptedPrivateKey() {
            return data
        }
